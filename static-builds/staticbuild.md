...
git clone https://github.com/naldproject/naldcoin
mkdir -p release
cd naldcoin/depends
...
sudo find ${DISTNAME}/bin -type f -executable -exec ../naldcoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../naldcoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo nald-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find nald-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/naldcoin
...
cp -f nald-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf naldcoin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/naldcoin/nald-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/nald-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer nald-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer nald-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf naldcoin/contrib/windeploy ~/sign/win32
...
#transfer nald-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/naldcoin/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/naldproject/naldcoin
mkdir ~/naldcoin/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder naldcoin/depends/SDKs
cd naldcoin/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/naldcoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Naldcoin-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/nald-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer nald-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
