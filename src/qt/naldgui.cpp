...
const std::string NaldcoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString NaldcoinGUI::DEFAULT_WALLET = "~Default";

NaldcoinGUI::NaldcoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void NaldcoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Naldcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nald: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to NALD addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Naldcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Naldcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a nald: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Naldcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void NaldcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void NaldcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** NALD START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/naldcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* naldLabelLayout = new QVBoxLayout(toolbarWidget);
        naldLabelLayout->addWidget(label);
        naldLabelLayout->addWidget(toolbar);
        naldLabelLayout->setDirection(QBoxLayout::TopToBottom);
        naldLabelLayout->addStretch(1);
...
void NaldcoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool NaldcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool NaldcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void NaldcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void NaldcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void NaldcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void NaldcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void NaldcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NaldcoinGUI::optionsClicked()
{
...
void NaldcoinGUI::showDebugWindow()
{
...
void NaldcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void NaldcoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void NaldcoinGUI::openClicked()
{
...
void NaldcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NaldcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NaldcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NaldcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NaldcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NaldcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** NALD START */
void NaldcoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void NaldcoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void NaldcoinGUI::gotoManageAssetsPage()
{
...
void NaldcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Naldcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void NaldcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NaldcoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void NaldcoinGUI::updateHeadersSyncProgressLabel()
{
...
void NaldcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void NaldcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Naldcoin"); // default title
...
void NaldcoinGUI::changeEvent(QEvent *e)
{
...
void NaldcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NaldcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NaldcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "NALD")
        msg += tr("Amount: %1\n").arg(NaldcoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(NaldcoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void NaldcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NaldcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NaldcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NaldcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NaldcoinGUI::setHDStatus(int hdEnabled)
{
...
void NaldcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NaldcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NaldcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NaldcoinGUI::detectShutdown()
{
...
void NaldcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void NaldcoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void NaldcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(NaldcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void NaldcoinGUI::subscribeToCoreSignals()
{
...
void NaldcoinGUI::unsubscribeFromCoreSignals()
{
...
void NaldcoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<NaldcoinUnits::Unit> units = NaldcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NaldcoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(NaldcoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (NaldcoinUnits::Unit u : NaldcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(NaldcoinUnits::name(u)), this);
...
{
    setText(NaldcoinUnits::name(newUnits));
}
...
void NaldcoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=NALDBTC"));
