...
#include "naldaddressvalidator.h"
#include "naldunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Naldcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new NaldcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new NaldcoinAddressCheckValidator(parent));
}
...
bool parseNaldcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no nald: URI
    if(!uri.isValid() || uri.scheme() != QString("nald"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!NaldcoinUnits::parse(NaldcoinUnits::NALD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseNaldcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert nald:// to nald:
    //
    //    Cannot handle this later, because nald:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nald://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "nald:");
    }
    QUrl uriInstance(uri);
    return parseNaldcoinURI(uriInstance, out);
}

QString formatNaldcoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("nald:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(NaldcoinUnits::format(NaldcoinUnits::NALD, info.amount, false, NaldcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openNaldcoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(NALD_CONF_FILENAME);
    ...
    /* Open nald.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Naldcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Naldcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Naldcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Naldcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "nald.desktop";
    return GetAutostartDir() / strprintf("nald-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a nald.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Naldcoin\n";
        else
            optionFile << strprintf("Name=Naldcoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the nald app
...
bool GetStartOnSystemStartup()
{
    CFURLRef naldAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (naldAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, naldAppUrl);

    CFRelease(naldAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add nald app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, naldAppUrl, nullptr, nullptr);
    }
