...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** NALD START */
    case OP_NALD_ASSET              : return "OP_NALD_ASSET";
    /** NALD END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_NALD_ASSET) { // OP_NALD_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == NALD_R) { // Check to see if NALD starts at 27 ( this->size() < 105)
                if ((*this)[28] == NALD_V)
                    if ((*this)[29] == NALD_N)
                        index = 30;
            } else {
                if ((*this)[28] == NALD_R) // Check to see if NALD starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == NALD_V)
                        if ((*this)[30] == NALD_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == NALD_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == NALD_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == NALD_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == NALD_R) {
