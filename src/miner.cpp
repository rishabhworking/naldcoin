...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    if (nHeight <= DEV_FUND_UNTIL) {
      ...
      coinbaseTx.vout[1].scriptPubKey = GetScriptForDestination(CNaldcoinAddress(address).Get());
...
void static NaldcoinMiner(const CChainParams& chainparams)
{
    LogPrintf("NaldcoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nald-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("NaldcoinMiner -- Wallet not available\n");
    }
        ...
        while (true) {
            ...
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    LogPrintf("NaldcoinMiner -- waiting for connections and blocks downloaded\n");
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("NaldcoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("NaldcoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("NaldcoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("NaldcoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateNaldcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&NaldcoinMiner, boost::cref(chainparams)));
    }
