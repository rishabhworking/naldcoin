...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of NALDCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("NALD", type));
        BOOST_CHECK(!IsAssetNameValid("NALD", type));
        BOOST_CHECK(!IsAssetNameValid("NALDCOIN", type));

        //- Versions of NALDCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("NALD.COIN", type));
        BOOST_CHECK(IsAssetNameValid("NALD_COIN", type));
        BOOST_CHECK(IsAssetNameValid("NALDSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERNALD", type));
        BOOST_CHECK(IsAssetNameValid("NALDSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDENALD", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_NALDS", type));
        BOOST_CHECK(IsAssetNameValid("SENALDOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("NALD", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
