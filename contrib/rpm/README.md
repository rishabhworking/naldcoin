...
The RPM spec file provided here is for Naldcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://nald.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Naldcoin git repository](https://github.com/naldcoin/naldcoin) but are not part of
...
Currently only `Source10` is used, for the example `nald.conf` file.
...
in the nald git repository. At present only an SVG version of the Naldcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nald
...
needed when 0.12.1 is released, a proper fix is already in the Naldcoin git
master branch.
...
idea to build Naldcoin Core against a static version of that library compiled
...
The `nald.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nald-server` package. At this point, most RPM based
...
The `nald-server` package should have SELinux support. How to properly do
...
Naldcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/nald/). He has been
packaging Naldcoin for Fedora at least since 2012.
...
4. I renamed the `nald` package that contains the Qt GUI to `nald-core` as
that appears to be how the general population refers to it, in contrast to
`nald-xt` or `nald-classic`. I wanted to make sure the general population
